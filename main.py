    ...
    html_content += f"""
        ...
        <script>
            ...
            function openLoginModal() {{
                document.getElementById('loginMessage').style.display = 'none'; // clear old errors
            ...
            async function toggleFavorite(event, ean) {{
                ...
                if (!window.auth || !window.auth.currentUser) {{
                    openLoginModal(); // Pop open the new UI modal if they aren't logged in!
                    return;
                }}
